    ...
    private void onAlgorithmRun() {
        try {
            ...
            if (algorithm.equals("Huffman coding")) {
                ...
                String pattern = JOptionPane.showInputDialog(frame, "Enter search pattern:");
                if (pattern == null) {
                    compressionOutput.append("KMP search cancelled.\n");
                } else {
                    int index = KMP.search(pattern, text);
                    if (index == -1) {
                        compressionOutput.append("Pattern not found.\n");
                    } else {
                        compressionOutput.append("First occurrence at index: " + index + "\n");
                    }
                }
                ...
                String pattern = JOptionPane.showInputDialog(frame, "Enter search pattern:");
                if (pattern == null) {
                    compressionOutput.append("Boyer-Moore search cancelled.\n");
                } else {
